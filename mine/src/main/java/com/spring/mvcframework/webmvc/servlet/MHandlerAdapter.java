...
import com.spring.mvcframework.annotation.MRequestParam;
...
public class MHandlerAdapter {
    public boolean supports(Object handler){ return (handler instanceof MHandlerMapping);}
    MModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
        MHandlerMapping handlerMapping = (MHandlerMapping)handler;
        ...
        boolean isModelAndView = handlerMapping.getMethod().getReturnType() == MModelAndView.class;
        if(isModelAndView){
